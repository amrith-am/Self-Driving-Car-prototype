...
class KeyboardControl(object):
    ...
    def parse_events(self, world, keys):
        ...
        if not self._autopilot_enabled:
            self._parse_keys( key, int(round(time.time() * 2000)) )  # pygame.key.get_pressed(), clock.get_time())
            world.vehicle.apply_control(self._control)

    def _parse_keys(self, keys, milliseconds):
        self._control.throttle = 2.0 if keys == ord('w') else 0.0
...
class ObstacleSensor(object):
    def __init__(self, parent_actor, hud):
        ...
        self._history = []
        self._event_count = 0
        self.sensor_transform = carla.Transform(carla.Location(x=1.6, z=1.7), carla.Rotation(yaw=0))
    ...
    def _on_obstacle(weak_self, event):
        ...
        print('Obstacle Detected %r at distance %u' % (actor_type, event.distance))
        if(self._hud is not None):
            self._hud.notification('Obstacle Detected %r at distance %u' % (actor_type, event.distance))
        else:
            print('Obstacle Detected %r at distance %u' % (actor_type, event.distance)) 
...
START_POSITION = carla.Transform(carla.Location(x=220.0, y=-1.85, z=0.5))
...
class World(object):
    def __init__(self, carla_world, hud, args):
        ...
        self.ObstacleSensor = ObstacleSensor(self.vehicle, self.hud)
    ...
    def destroy(self):
        for actor in [self.camera_manager.sensor, self.ObstacleSensor.sensor, self.vehicle]:
