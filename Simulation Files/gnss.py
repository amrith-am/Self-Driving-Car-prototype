...
class HUD(object):
    ...
    def render(self, image):
        if(image is not None):
            cv2.putText(image, self._text, (5,30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255))
...
class Gnsssensor(object):
    def __init__(self, parent_actor, hud):
        self.sensor = None
        self.sensor_transform = carla.Transform(carla.Location(0,0,0), carla.Rotation(0,0,0))
        self._parent = parent_actor
        self._hud = hud
        world = self._parent.get_world()
        bp = world.get_blueprint_library().find('sensor.other.gnss')
        bp.set_attribute("sensor_tick", str(3.0))
    ...
    def _gnss_callback(weak_self, event):
        ...
        if(self._hud is not None):
            self._hud.notification(str(event))
            print(str(event))
...
START_POSITION = carla.Transform(carla.Location(x=180.0, y=-1.5, z=40.0))
...
class World(object):
    def __init__(self, carla_world, hud, args):
        ...
        self.gnss_sensor = Gnsssensor(self.vehicle, self.hud)
    ...
    def destroy(self):
        for actor in [self.camera_manager.sensor, self.gnss_sensor.sensor, self.vehicle]:
